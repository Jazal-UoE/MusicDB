...
const ArtistConnection = () => {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        console.log(`Fetching songs for ${artist1} and ${artist2}`);
        const response = await fetch(
          `http://localhost:8000/api/artists2/contributionConnection/?singer1=${encodeURIComponent(
            artist1
          )}&singer2=${encodeURIComponent(artist2)}`
        );
  ...
  const handleArtistClick = (artistName) => {
    if (
      artistName.toLowerCase() === artist1.toLowerCase() ||
      artistName.toLowerCase() === artist2.toLowerCase()
    ) {
      navigate(`/artist/${artistName}`);
    } else {
      navigate(`/artistConnectionThree/${artist1}/${artist2}/${artistName}`);
    }
  };
  ...
  return (
    <div className="container mt-5">
      ...
      <h1 className="mb-4 text-center custom-title">
        Collaborations: {artist1.toUpperCase()} and {artist2.toUpperCase()}
      </h1>
...
export default ArtistConnection;
